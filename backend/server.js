...
app.get("/tickets", (req, res) => {
  ...
  db.query(sql, (error, results) => {
    if (error) return res.status(500).json({ error: "Failed to get tickets" });
    res.json(results);
  });
});

// GET /tickets/open  ← must be BEFORE /tickets/:id
...
// GET /tickets/:id  ← must be AFTER /tickets/open
app.get("/tickets/:id", (req, res) => {
  const ticketId = req.params.id;
  const sql = "SELECT * FROM tickets WHERE id = ?";
  db.query(sql, [ticketId], (error, results) => {
    if (error) return res.status(500).json({ error: "Failed to get ticket" });
    if (results.length === 0)
      return res.status(404).json({ error: "Ticket not found" });
    res.json(results[0]);
  });
